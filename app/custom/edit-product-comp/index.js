...
export const EditProductModal = ({ onClickCancel, id }) => {
  ...
  const { values, handleChange, setFieldValue, handleSubmit } = useFormik({
    initialValues: {
      name: getSingleProduct?.data.name ?? "",
      brand: getSingleProduct?.data.brand ?? "",
      sellingPrice: getSingleProduct?.data.selling_price ?? "",
      costPrice: getSingleProduct?.data.cost_price ?? "",
      stock: getSingleProduct?.data.stock ?? "",
      weight: getSingleProduct?.data.weight ?? "",
      pricingType: getSingleProduct?.data?.pricing_type ?? "",
      category: getSingleProduct?.data.category_name ?? "",
    },
    onSubmit,
  ...
  return (
    <div className={styles.AddModal}>
      <div className={styles.content}>
        ...
        <div className="text-[22px] font-[600]">Edit Product</div>
